...
ENV_DEFAULTS = {
    "APPLICATION_NAME" : "Sablier Station",
    "APPLICATION_DESCRIPTION" : "Welcome to Sablier Station",
    "APPLICATION_DETAILS" : "Easily configure Traefik and Sablier",
...
docker_handler = None
if app.config.get("DOCKER_AUTO_SYNC", True):
    try:
        docker_handler = init_docker_handler(app)
        app.docker_handler = docker_handler
        logging.info("Docker event handler initialized")
    except ImportError as e:
        logging.warning(f"Docker handler not available: {e}")
    except Exception as e:
        logging.error(f"Failed to initialize Docker handler: {e}")
