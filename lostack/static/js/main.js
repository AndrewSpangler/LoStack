...
function createFallbackIcon(size = 50) {
    ...
    Object.assign(fallbackElement.style, {
        width: `${size}px`,
        height: `${size}px`,
        background: 'linear-gradient(180deg, rgb(var(--color-logo-start, 108, 117, 125)), rgb(var(--color-logo-stop, 73, 80, 87)))',
    ...
    fallbackElement.className = 'fallback-icon';
    fallbackElement.setAttribute('data-icon-type', 'fallback');
...
// Global terminal modal instance
const terminalModal = new TerminalModal();

// Global function to launch the terminal modal
function launchTerminalModal(streamUrl) {
    terminalModal.launch(streamUrl);
}

// Animate a button with a spinner icon
