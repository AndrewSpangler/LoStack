...
class FileBrowser:
    ...
    def __init__(self, app, base_directory, url_prefix='/files'):
        self.app = app
    ...
    def _create_blueprint(self):
        """Create the Flask blueprint with routes"""
        self.blueprint = Blueprint('file_browser', __name__)
        
        # Store reference to the extension instance in the blueprint
    ...
    def _render_file_browser(self, current_path):
    ...
    def _serve_file_content(self, file_path):
        """Serve file contents"""
        if not file_path:
    ...
    @staticmethod
    def _is_binary_file(path, blocksize=1024):
...
def init_filebrowser(app, base_directory="/docker", url_prefix="/files"):
