...
def depot_package_install(package_name, result_queue, complete=True) -> Response:
    """Stream response to handle adding depot package"""    
    result_queue.put_nowait("Adding depot package to lostack compose file.")
    docker_handler = get_docker_handler()
    try:
        services = docker_handler.add_depot_package(package_name, result_queue)
    except Exception as e:
        import traceback

        result_queue.put_nowait(f"Error adding package services to compose: {traceback.print_exc(e)}")
    ...
    return docker_compose_up(
        services, 
        result_queue, 
        compose_file=str(docker_handler.lostack_file), 
        complete=complete
    )

def depot_package_remove(service_db_id:int, result_queue, complete=True) -> Response:
    docker_handler = get_docker_handler()
    try:
        return docker_handler.remove_depot_package(service_db_id, result_queue)
    except Exception as e:
        result_queue.put_nowait(f"Error removing service from compose: {e}")
        time.sleep(1) # Wait for queue flush before context exit
        return
